...
base_2 <- function(Posts){
    ...
    s1 <- aggregate(s$Score, s[, c("Month", "Year")], length)
    ...
    s2 <- aggregate(s$Score, s[, c("Month", "Year")], max)
    colnames(s2)[3] <- "MaxScore"
    # dodajemy MaxScore do s1)
    s1["MaxScore"] <- s2$MaxScore   
    # wybieramy tylko te wejscia co maja PostsNumber > 1000
    s1 <- s1[idxs, ]
    # resetujemy indeks i ustawiamy kolejnosc kolumn
    s1 <- s1[, c("Year", "Month", "PostsNumber", "MaxScore")]
    rownames(s1) <- NULL
    # zwracamy wynik
    return (s1) 
}
